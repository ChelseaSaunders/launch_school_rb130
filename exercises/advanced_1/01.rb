...
multiplying all the integers from 1 up to the n value together. Thus,
5! is 5 * 4 * 3 * 2 * 1, or 120.
...
- test by printing out the first 7 factorial values, starting with zero
...
Discussion
Most of the work for solving this problem relates to creating our new Enumerator
object. We can get a good start at understanding how to do this by referencing
the Enumerator::new method. It shows how to define a new Enumerator that
iterates over Fibonacci numbers. We'll start with that as a template, and alter
it to iterate over factorials instead. For iterating over factorials, we'll need
two variables: one to track which number factorial we are on, and another store
the cumulative value of all prior factorials. We use accumulator for the latter,
and we use number for the former.

When defining an external iterator, we set the next value in the iteration by
calling yielder << value. In this case, we use accumulator for value; it sets
the current factorial value.

To use our Enumerator object as an internal and external iterator, we utilize
two different groups of code. First, we have 3 calls to the Intger#times method;
this code uses an external iterator (factorial.next). Lastly, we have an
#each_with_index loop; this code uses an internal iterator.
...
when we using an external iterator. That's something internal iterators do.
...
Next, we move on to our #each_with_index call. #each_with_index is designed to
work with Enumerator objects, so it uses an internal iterator. In particular,
#each_with_index knows that it needs to call #rewind each time you invoke it.
Therefore, when we invoke #each_with_index here, it automatically calls #rewind,
and outputs the desired results.
